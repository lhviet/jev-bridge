...
Logs are sanitised as they are written ([`run.mjs`](run.mjs), `sanitize()`): the home directory becomes `~`, thinking signatures are dropped, and the init record keeps only this server's tools and commands. Nothing else is changed.
